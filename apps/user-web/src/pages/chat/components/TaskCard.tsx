      ...
      <div className="flex min-w-0 items-start gap-2">
        ...
        <div className="min-w-0">
          <div className="truncate text-[12px] font-semibold text-(--text-strong)">
            {error.stage || t('pages.chat.task.error.execution')}
          </div>
    ...
    >
      <div className="flex min-h-0 w-full flex-col">
        <div className="mb-2 flex shrink-0 items-start justify-between gap-2">
          <div className="min-w-0">
            <div className="line-clamp-1 text-[14px] font-semibold leading-5 text-(--text-strong)">
              {task.web_url ? (
                <Tooltip placement="top" title={t('pages.chat.task.preview')}>
        ...
        <div className="grid shrink-0 gap-1.5 rounded-md bg-(--control-bg) px-2.5 py-2">
          <TaskInfoRow label={t('pages.chat.task.tenant')}>
            <Typography.Text
              copyable
          ...
          <TaskInfoRow label={t('pages.chat.task.node')}>
            <span className="max-w-37.5 truncate text-(--text)">
              {task.current_node || '-'}
            </span>
          </TaskInfoRow>
          <TaskInfoRow label={t('pages.chat.task.updated')}>
        ...
        <div className="mt-2 flex shrink-0 flex-wrap items-center gap-1.5">
          ...
          {task.retry_count > 0 ? (
            <Tag color="orange" className="m-0! text-[12px]! leading-4.5!">
              {t('pages.chat.task.retry', { count: task.retry_count })}
            </Tag>
          ) : null}
          {isApproval ? (
            <Tag
