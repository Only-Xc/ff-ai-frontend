      ...
      <div>
        ...
        <Space size="large" style={{ marginBottom: 24 }}>
          <Statistic
            ...
            valueRender={() => (
              <Tag color={RESULT_COLORS[evaluation.result] ?? 'default'}>
                {evaluation.result}
              </Tag>
            )}
          />
          ...
          <Statistic
            ...
            value={evaluation.agent_id}
          />
          <Statistic
            title={t('pages.grc.evaluations.triggerType')}
            value={evaluation.trigger_type}
          />
        </Space>
        ...
        >
          <Descriptions.Item label={t('pages.grc.evaluations.startedAt')}>
            {evaluation.started_at
              ? dayjs(evaluation.started_at).format('YYYY-MM-DD HH:mm:ss')
              : '-'}
          </Descriptions.Item>
          <Descriptions.Item label={t('pages.grc.evaluations.completedAt')}>
            {evaluation.completed_at
              ? dayjs(evaluation.completed_at).format('YYYY-MM-DD HH:mm:ss')
              : '-'}
          </Descriptions.Item>
        ...
        {results.length > 0 ? (
          <Table
            ...
            columns={[
              {
                title: t('pages.grc.evaluations.ruleCode'),
                dataIndex: 'rule_code',
                key: 'rule_code',
                width: 150,
              ...
              {
                title: t('pages.grc.evaluations.ruleName'),
                dataIndex: 'rule_name',
                key: 'rule_name',
                width: 150,
                ellipsis: true,
              ...
              {
                title: t('pages.grc.evaluations.blockOnFail'),
                dataIndex: 'block_on_fail',
                key: 'block_on_fail',
                width: 110,
                render: (v) =>
                  v ? t('pages.grc.common.yes') : t('pages.grc.common.no'),
              },
            ]}
          />
        ) : (
