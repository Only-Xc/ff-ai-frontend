...
export default function AdminPluginCarrier() {
  ...
  const { pluginId: routePluginId = '', '*': routePluginPath = '' } = useParams()
  ...
  if (sessionQuery.isError || !sessionQuery.data) {
    return (
      <Result
        ...
        extra={[
          <Button key="back" icon={<ArrowLeftOutlined />} onClick={() => void navigate('/plugins')}>
      ...
      <header className="flex shrink-0 items-center justify-between border-b border-(--border) px-4 py-2">
        <Space>
          ...
          <div className="min-w-0">
            <Typography.Text className="block font-medium">{pluginId}</Typography.Text>
