...
<template>
  <div class="graph-wrap" ref="wrap" :class="{ 'is-fullscreen': fullscreen }">
    ...
    <div v-if="!entities.length" class="graph-empty">
      <SvgIcon name="graph" :size="38" class="empty-icon" />
      <div class="empty-title">暂无图谱数据</div>
      <div class="empty-desc">该来源尚未构建图谱</div>
    </div>
...
.graph-wrap {
  ...
  box-shadow:
    0 0 0 1px var(--graph-panel-border),
    var(--graph-shadow);
...
.empty-icon { color: var(--graph-muted); opacity: 0.75; }
.empty-title { color: var(--graph-text); font-size: 1rem; font-weight: 600; }
.empty-desc { color: var(--graph-muted); font-size: 0.82rem; max-width: 280px; text-align: center; line-height: 1.5; }
...
.stats-title {
  color: var(--graph-accent); font-size: 0.7rem; font-weight: 600; text-transform: uppercase;
...
.stat-item {
  ...
  padding: 4px 6px;
  background: color-mix(in srgb, var(--graph-panel) 82%, var(--graph-bg));
  border-radius: 6px;
}
.stat-val { font-size: 0.95rem; font-weight: 700; color: var(--graph-text); line-height: 1.2; }
.stat-lbl { font-size: 0.62rem; color: var(--graph-muted); font-weight: 500; }
.stats-toggle {
  position: absolute; top: 6px; right: 8px;
  background: none; border: none; color: var(--graph-muted); cursor: pointer;
  font-size: 0.85rem; padding: 2px 6px; border-radius: 4px;
}
.stats-toggle:hover { color: var(--graph-accent); }
.stats-show-btn {
  position: absolute; top: 12px; left: 12px; z-index: 3;
  background: var(--graph-panel);
  border: 1px solid var(--graph-panel-border);
  border-radius: 10px; color: var(--graph-accent); font-size: 1rem;
...
.ctrl-btn {
  ...
  border: 1px solid var(--graph-panel-border);
  border-radius: 10px;
  color: var(--graph-accent); font-size: 0.85rem; font-weight: 600;
...
.hint-bar {
  ...
  font-size: 0.65rem; color: color-mix(in srgb, var(--graph-muted) 70%, transparent);
