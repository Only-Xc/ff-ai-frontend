...
export class ForceGraph {
  ...
  private render() {
    ...
    bg.addColorStop(0, dark ? '#0A0E1A' : '#F8FAFC')
    bg.addColorStop(0.55, dark ? '#0E1424' : '#F3F5FA')
    bg.addColorStop(1, dark ? '#131B30' : '#EEF1F7')
    ctx.fillStyle = bg
    ctx.fillRect(0, 0, W, H)
    // radial glow from center
    const gl = ctx.createRadialGradient(W / 2, H / 2, 0, W / 2, H / 2, Math.max(W, H) * 0.7)
    gl.addColorStop(0, dark ? 'rgba(45,225,194,0.06)' : 'rgba(107,67,247,0.05)')
    gl.addColorStop(1, dark ? 'rgba(10,14,26,0)' : 'rgba(238,241,247,0)')
    ctx.fillStyle = gl
    ctx.fillRect(0, 0, W, H)

    this.drawStars(ctx, dark)
    ...
    for (const nd of this.nodes) {
      const lit = isLit(nd)
      this.drawNode(ctx, nd, lit ? 1 : 0.18, dark)
    }
  }
  ...
  private drawNode(ctx: CanvasRenderingContext2D, nd: GNode, alpha: number, dark: boolean) {
    const p = this.toScreen(nd.x, nd.y)
    ...
    if (showLabel && alpha > 0.3) {
      ...
      const ly = p.y + r + 4
      ctx.shadowColor = dark ? 'rgba(0,0,0,0.85)' : 'rgba(255,255,255,0.9)'
      ctx.shadowBlur = dark ? 4 : 3
      ctx.fillStyle = nd === this.hoverNode
        ? this.mix(nd.color, dark ? '#ffffff' : '#111827', dark ? 0.6 : 0.45, 1)
        : dark ? `rgba(226,232,240,${alpha})` : `rgba(31,41,55,${alpha})`
      ctx.fillText(nd.name, p.x, ly)
      ctx.shadowBlur = 0
    }
  }

  private drawStars(ctx: CanvasRenderingContext2D, dark: boolean) {
    ctx.globalCompositeOperation = dark ? 'lighter' : 'source-over'
    for (const st of this.stars) {
      const tw = 0.5 + 0.5 * Math.sin(this.time * st.tw + st.x)
      ctx.fillStyle = dark
        ? `rgba(180,210,255,${st.a * tw})`
        : `rgba(89,99,119,${st.a * tw * 0.28})`
      ctx.beginPath()
