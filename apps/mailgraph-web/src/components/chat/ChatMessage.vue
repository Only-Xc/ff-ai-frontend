  ...
  <div class="msg-wrapper" :class="message.role">
    ...
    <div class="msg-body">
      ...
      <div class="msg-content" :class="message.role">
        <div class="msg-text" data-no-ui-translate v-html="renderedContent"></div>
...
.assistant-avatar-inner {
  background: var(--p);
  color: #fff;
  box-shadow: 0 2px 8px rgba(107, 67, 247, 0.22);
  width: 40px;
  height: 40px;
  position: relative;
}
...
.msg-content.user {
  ...
  color: #fff;
  max-width: 85%;
  box-shadow: 0 2px 8px rgba(107, 67, 247, 0.2);
  display: inline-block;
}
.msg-text {
  white-space: normal;
...
/* Keep markdown readable inside the primary user bubble. */
...
.cursor-bar {
  ...
  border-radius: 2px;
  box-shadow: 0 0 6px rgba(107, 67, 247, 0.35);
...
.step-dot.done {
  background: var(--p);
  box-shadow: 0 0 8px rgba(107, 67, 247, 0.3);
}
.step-dot.done::after {
  content: '';
  position: absolute;
  inset: -3px;
  border-radius: 50%;
  border: 1.5px solid rgba(107, 67, 247, 0.22);
...
.step-pulse {
  ...
  border-radius: 50%;
  background: var(--p);
  box-shadow: 0 0 0 rgba(107, 67, 247, 0.45);
  animation: pulseDot 1.2s ease-in-out infinite;
  flex-shrink: 0;
}
@keyframes pulseDot {
  0% { box-shadow: 0 0 0 0 rgba(107, 67, 247, 0.55); }
  50% { box-shadow: 0 0 0 6px rgba(107, 67, 247, 0); }
  100% { box-shadow: 0 0 0 0 rgba(107, 67, 247, 0); }
}
...
.score-high { color: var(--p); background: var(--p-bg); }
