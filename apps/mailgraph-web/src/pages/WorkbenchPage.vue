...
async function handleDelete() {
  ...
  const confirmed = window.confirm(
    `${t('确认删除选中的邮件？')}\n${t('将删除工作台记录和知识索引，不会删除 IMAP 服务器中的原始邮件。')}`,
  )
  if (!confirmed) return

  deleting.value = true
  try {
    const result = await mailsApi.delete(ids)
    processLogs.value = [`${t('已删除')} ${result.deleted} ${t('封邮件')}`]
    if (result.not_found.length) {
      processLogs.value.push(`${t('未找到')} ${result.not_found.length} ${t('封邮件')}`)
    }
    selectedIds.value = new Set()
    await refreshAll()
  } catch (e: any) {
    processLogs.value = [`${t('删除失败')}: ${e?.message || String(e)}`]
  } finally {
...
onMounted(() => {
  refreshAll()
  // The integrated platform uses authenticated polling because native
  // EventSource cannot attach the platform Authorization header.
  pollTimer = setInterval(() => {
    refreshGraphStatus()
