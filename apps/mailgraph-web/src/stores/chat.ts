...
export const useChatStore = defineStore('chat', () => {
  ...
  async function sendMessage(
    question: string,
    sourceFilter?: QuerySourceFilter,
    sourceScope: QuerySourceScope = { type: 'global', id: '', label: '全部知识来源' },
  ) {
    ...
    await conversationsApi.addMessage(sid, 'user', question, { source_scope: sourceScope })
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      queryApi.run(question, sid, sourceFilter, {
        onProgress(data: any) {
          if (data.token) {
        ...
        onComplete(data: any) {
          ...
          if (!result.source_scope) result.source_scope = sourceScope
