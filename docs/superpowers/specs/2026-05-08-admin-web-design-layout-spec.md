...
管理端页面优先保证滚动稳定、信息扫描效率和长时间操作舒适度。页面级视觉效果遵循以下规则：

- 应用级 Layout、Header、Sidebar 作为全局骨架，样式保持稳定；页面滚动闪烁、白闪、抖动等问题优先在具体页面和页面组件内处理。
- 高频滚动区域内的大面积卡片、表格容器、图表容器、代码块和详情面板使用实体背景，例如 `var(--panel)`、`var(--card)` 或 `var(--ant-color-bg-container)`。
- 滚动内容区域里的大面积 `backdrop-filter`、`backdrop-blur`、透明渐变和多层透明背景需要收敛为实体背景、细边框和轻阴影。
- 页面级卡片、代码块、图表 body、可滚动面板可使用 `contain: paint` 隔离绘制范围。
- 页面内滚动容器可使用 `scrollbar-gutter: stable` 稳定滚动条占位。
- 滚动页面内的阴影以轻量阴影为主，例如 `0 1px 2px rgb(15 23 42 / 0.04)`；核心模块需要层级时再使用中等阴影。
- 滚动区域内的 hover 动效优先使用边框色、背景色等低成本变化；多卡片列表减少 `box-shadow`、`transform` 在滚动场景中的过度使用。
- 图表、代码块、表格和日志容器需要稳定高度或最大高度，避免滚动过程触发连续测量和布局变化。
