...
| 入库概览 | `document_count`、`chunk_count` | 使用代理响应直接提供的聚合字段    |
| 切片方式 | `chunk_method`                  | 使用单一映射显示标签              |
| 更新时间 | `update_time`                   | RAGFlow 更新时间戳                |
| 操作     | -                               | 更多菜单：编辑、删除              |
...
| 指标           | 来源              | 说明               |
| -------------- | ----------------- | ------------------ |
| 文档总数       | `document_count`  | 知识库聚合字段     |
| chunk 总数     | `chunk_count`     | 知识库聚合字段     |
| Embedding 模型 | `embedding_model` | 缺失显示默认或 `-` |
| Chunk Method   | `chunk_method`    | 单一映射展示       |
| 最近更新时间   | `update_time`     | 格式化展示         |
...
| 列       | 字段           | 说明                              |
| -------- | -------------- | --------------------------------- |
| 文件名   | `name`         | 主标题，副文本显示 `id`，支持复制 |
| 入库阶段 | `run`          | 使用 RAGFlow 对外状态枚举         |
| Chunk 数 | `chunk_count`  | 数字右对齐，无值显示 `-`          |
| 文件大小 | `size`         | 格式化为 KB/MB                    |
| Parser   | `chunk_method` | RAGFlow 对外切片方式字段          |
| 上传时间 | `create_time`  | 格式化展示                        |
| 最近解析 | `update_time`  | 格式化展示                        |
| 操作     | -              | 解析、重试、删除、查看详情        |
...
- `packages/api` 按代理的固定响应层级解包为 `{ data, count }`。
...
| 列       | 字段          | 说明                      |
| -------- | ------------- | ------------------------- |
| 文件名   | `name`        | 主标题，副文本显示 `id`   |
| 解析状态 | `run`         | 使用 RAGFlow 对外状态枚举 |
| 切片数   | `chunk_count` | 切片数量                  |
| 大小     | `size`        | 格式化为 KB/MB            |
| 上传时间 | `create_time` | RAGFlow 创建时间戳        |
| 操作     | -             | 解析、删除                |
...
- 命中文本使用 `chunks[].content`。
- 来源文档使用 `chunks[].document_keyword`。
- 分数使用 `chunks[].similarity`。
- 片段 ID 使用 `chunks[].id`。
...
共享 API 层按端点的固定响应结构解包，页面直接消费 RAGFlow 对外字段。
...
- 平台响应的 `data` 内保存 RAGFlow 原始响应。
- 知识库列表读取 `data.data` 和 `data.total_datasets`。
- 文档列表读取 `data.data.docs` 和 `data.data.total`。
- 知识库详情读取 `data.data[0]`。
- 文档详情读取 `data.data.docs[0]`。
- 检索读取 `data.data.chunks`、`data.data.total`、`data.data.doc_aggs`。
- 响应结构变化通过类型检查或运行错误暴露，页面不递归查找候选字段。
...
- 文档入库状态按 `UNSTART`、`RUNNING`、`CANCEL`、`DONE`、`FAIL` 展示。
- 解析失败文档支持重试解析。
- 文档删除批量请求体为 `{ "ids": [...] }`。
- 指定知识库检索请求体包含 `question` 和 `top_k`。
